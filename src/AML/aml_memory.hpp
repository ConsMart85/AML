...
#ifndef _INCLUDE_AML_MEMORY_2012_9_5
#define _INCLUDE_AML_MEMORY_2012_9_5
...
public:
	...
	void SetMemory( const T* pbuf, aml_uint pos, aml_uint count)
	{
		if(!pbuf)
		{
		...
		if((this->count - pos) < count)
		{

			throw aml_exception_overflow("Size of destination's memory  is small.");

		}
		memcpy(data + pos, pbuf, count * sizeof(T));		
	}


	T& operator ()(aml_uint pos)
	{
		return *(data + pos);
	}

	aml_allocator<T>& operator =(const aml_allocator<T>&  src)
	{
		Resize(src.GetCount());
		count = src.GetCount();
		SetMemory(src.data, count);	
		return *this;
	}
